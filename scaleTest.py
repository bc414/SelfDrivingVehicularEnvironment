...
#from leastSquares import calcMatrix 
import socket
import sys
import json
...
def callRansac():
    ...
    np.save("validPoints", isValid)
    np.savetxt("originCenters.txt",centerCoordinates)
    np.savetxt("validCenters.txt",isValid)
...
#def getSocketMatrices():
#send the center points and isValid matrix from server
#obtain valid points between the two and create two matrices with corresponding points
#callLeastSquares

#def callLeastSquares():
	#takes in the two center matrices as parameters and receives no parameters, only the npy files to node	
...
#Before the loop to save frames, we need to get a start time from the client
startTime = 0
...
while i<10:
    ...
    if output.strip() == "disconnect":
        ...
        #conn.send("dack")
        #print("after sending dack")
        break
    elif output:
        print "Message received from client:"
        startTime = json.loads(output)
        print(startTime)
...
delay = 2.0
print("time before for loop: ",time.time())
#while True:
for frameNumber in range(0,20): 
    ...
    if key in (27, ord("q")) or cv2.getWindowProperty(state.WIN_NAME, cv2.WND_PROP_AUTOSIZE) < 0:
        break
    time.sleep((frameNumber+1)*delay-time.time()+startTime)

    print("Current time since start: ",time.time() - startTime)
    #if key == ord("g"):
...
pipeline.stop()
callRansac()
#use socket connection to get the center coordinates getSocketMatrix this will automatically call leastSquares 
